...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader): # i is the batch-index
        ...
        if args.half: #half precision if specified
        ...
        #convert output and loss to float (Why?)
        output = output.float()
        loss = loss.float()
        
        # calculate accuracy
        prec1 = accuracy(output.data, target)[0] #note that this accuracy is the accuracy on the current batch.
        
        # record/update loss and accuracy for the current batch
...
def validate(val_loader, model, criterion):
    """
    Run evaluation
    Returns average accuracy on the entire dataset used for validation.
    """
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            # measure accuracy and update loss and accuracy
...
def accuracy(output, target, topk=(1,)): #one element tuple requires a comma, hence the default value of topk
